...
public partial class ElevationFilter : FilterViewModelBase
{
    [ObservableProperty]
    ...
    public override string DisplayName => "Elevation";
    ...
    public override IQueryable<ImageFile> ApplyFilter(LumidexDbContext dbContext, IQueryable<ImageFile> query)
    {
        if (MinValue is { } min)
        {
            ...
            query = query.Where(f => f.Elevation >= minimum);
        }

        if (MaxValue is { } max)
        {
            var maximum = (double)max;
            query = query.Where(f => f.Elevation <= maximum);
        }
