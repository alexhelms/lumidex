...
public partial class FocuserPositionFilter : FilterViewModelBase
{
    [ObservableProperty]
    ...
    public override string DisplayName => "Focuser Position";
    ...
    public override IQueryable<ImageFile> ApplyFilter(LumidexDbContext dbContext, IQueryable<ImageFile> query)
    {
        if (MinValue is { } min)
        {
            ...
            query = query.Where(f => f.FocuserPosition >= minimum);
        }

        if (MaxValue is { } max)
        {
            var maximum = (double)max;
            query = query.Where(f => f.FocuserPosition <= maximum);
        }
