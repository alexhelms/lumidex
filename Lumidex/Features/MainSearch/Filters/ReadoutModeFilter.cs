...
public partial class ReadoutModeFilter : FilterViewModelBase
{
    [ObservableProperty]
    public partial string? Name { get; set; }

    public override string DisplayName => "Readout Mode";
    ...
    public override IQueryable<ImageFile> ApplyFilter(LumidexDbContext dbContext, IQueryable<ImageFile> query)
    {
        if (Name is { Length: > 0 })
        {
            query = query.Where(f => EF.Functions.Like(f.ReadoutMode, $"%{Name}%"));
        }
