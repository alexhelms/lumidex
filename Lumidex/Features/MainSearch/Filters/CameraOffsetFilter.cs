...
public partial class CameraOffsetFilter : FilterViewModelBase
{
    [ObservableProperty]
    ...
    public override string DisplayName => "Camera Offset";
    ...
    public override IQueryable<ImageFile> ApplyFilter(LumidexDbContext dbContext, IQueryable<ImageFile> query)
    {
        if (MinValue is { } min)
        {
            ...
            query = query.Where(f => f.CameraOffset >= minimum);
        }

        if (MaxValue is { } max)
        {
            var maximum = (double)max;
            query = query.Where(f => f.CameraOffset <= maximum);
        }
