...
public partial class TemperatureFilter : FilterViewModelBase
{
    [ObservableProperty]
    ...
    public override string DisplayName => "Temperature";
    ...
    public override IQueryable<ImageFile> ApplyFilter(LumidexDbContext dbContext, IQueryable<ImageFile> query)
    {
        if (MinValue is { } min)
        {
            ...
            query = query.Where(f => f.Temperature >= minimum);
        }

        if (MaxValue is { } max)
        {
            var maximum = (double)max;
            query = query.Where(f => f.Temperature <= maximum);
        }
