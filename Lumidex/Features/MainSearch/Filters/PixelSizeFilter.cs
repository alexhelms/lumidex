...
public partial class PixelSizeFilter : FilterViewModelBase
{
    [ObservableProperty]
    ...
    public override string DisplayName => "Pixel Size";
    ...
    public override IQueryable<ImageFile> ApplyFilter(LumidexDbContext dbContext, IQueryable<ImageFile> query)
    {
        if (MinValue is { } min)
        {
            ...
            query = query.Where(f => f.PixelSize >= minimum);
        }

        if (MaxValue is { } max)
        {
            var maximum = (double)max;
            query = query.Where(f => f.PixelSize <= maximum);
        }
